...
@Composable
fun HomeScreen(navController: NavController, vm: NewsViewModel) {
    val news = vm.newsList
    Scaffold(
        topBar = {
            TopAppBar(
                title = {
                    Row {
                        ...
                        Icon(imageVector = Icons.Rounded.Person, contentDescription = null, modifier = Modifier.size(50.dp).clickable {
        ...
        floatingActionButton = {
            Icon(
                ...
                modifier = Modifier
                    ...
                    .size(60.dp)
                    .clickable {
                        navController.navigate(NavDestination.SAVED.route)
                    }
            )
        }
    ) { innerPadding ->
        ...
        LazyColumn(modifier = Modifier.padding(innerPadding)) {
            items(news.value) { item ->
                NewsView(item.title, item.description, item.author, item.publishedAt,
                    onCardClick = {
...
@Composable
fun NewsView( title: String, description: String?, author: String?, published : String?, onCardClick:() -> Unit, onFavClick:() -> Unit) {
